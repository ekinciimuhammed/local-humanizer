...
import { BrowserCheckError, CHECK_TIMEOUT_MS, runPublicCheck, validateText, validateProvider } from './checker.mjs';
...
async function readInput(req) {
  ...
  if (!body || typeof body !== 'object' || Array.isArray(body) || Object.keys(body).some(key => !['text', 'provider'].includes(key))) throw new BrowserCheckError('Only text and a supported provider are accepted.', 400, 'invalid_request');
  const provider = Object.hasOwn(body, 'provider') ? body.provider : 'zerogpt-web';
  validateProvider(provider); validateText(body.text); return {text: body.text, provider};
}
export function createBrowserChecker({ checkImpl = runPublicCheck, timeoutMs = CHECK_TIMEOUT_MS } = {}) {
  ...
  const server = http.createServer(async (req, res) => {
    try {
      ...
      const {text, provider} = await readInput(req);
      ...
      try {
        const result = await Promise.race([checkImpl(text, { signal: controller.signal, provider }), aborted]);
